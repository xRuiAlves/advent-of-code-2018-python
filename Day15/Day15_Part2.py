...
def reachable_positions(i, j, visited):
    global m
    pos = [[i, j]]
    if empty_cell(m[i][j+1]) and [i, j+1] not in visited + pos:
        more_pos = reachable_positions(i, j+1, visited + pos)
        pos += more_pos
    if empty_cell(m[i][j-1]) and [i, j-1] not in visited + pos:
        more_pos = reachable_positions(i, j-1, visited + pos)
        pos += more_pos
    if empty_cell(m[i+1][j]) and [i+1, j] not in visited + pos:
        more_pos = reachable_positions(i+1, j, visited + pos)
        pos += more_pos
    if empty_cell(m[i-1][j]) and [i-1, j] not in visited + pos:
        more_pos = reachable_positions(i-1, j, visited + pos)
        pos += more_pos
...
def attack(i, j):
    global m, num_elfs, num_goblins, elf_attack
    if (m[i][j].type == 'G'):
        m[i][j].hp -= elf_attack
    else:
        m[i][j].hp -= 3
...
initial_num_goblins = num_goblins
initial_num_elfs = num_elfs
elf_attack = 18
...
while True:
    ...
    round = -1
    num_goblins = initial_num_goblins
    num_elfs = initial_num_elfs
    elf_attack += 1
    print(elf_attack)
...
print('Elves attack power: ' + str(elf_attack))
